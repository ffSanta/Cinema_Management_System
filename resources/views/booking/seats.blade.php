...
@push('styles')
    <style>
        ...
        .seat-row {
            display: flex;
            align-items: center;
            gap: calc(var(--seat-size) * 0.14);
            margin-bottom: calc(var(--seat-size) * 0.14);
        }

        .row-label {
            width: calc(var(--seat-size) * 0.8);
            flex: 0 0 auto;
            text-align: center;
            font-weight: 600;
            color: #6c757d;
        ...
        .seat.gap {
            margin-left: calc(var(--seat-size) * 0.85);
        }

        /* เว้นระยะระหว่างโซน */
        .seat-row.zone-start {
            margin-top: calc(var(--seat-size) * 0.85);
        }

        /* สีที่นั่งตามโซน (ใช้ตัวแปร --zone จาก inline style ของแต่ละที่นั่ง) */
        .seat.available {
            background: #fff;
            border-color: var(--zone, #198754);
            color: var(--zone, #198754);
        }
        ...
        @media (hover: hover) {
            .seat.available:not(.selected):hover {
                background: var(--zone, #198754);
                color: #fff;
            }
        }
                ...
                <div class="card-body">
                    <div class="d-flex gap-3 mb-3">
                        <img src="{{ $showtime->movie->poster_url }}" alt="{{ $showtime->movie->title }}"
                            ...
                            style="width:70px; height:105px; object-fit:cover; background:#1a1a2e;">
                        <div>
                            <h5 class="card-title mb-1">{{ $showtime->movie->title }}</h5>
                            <div class="text-muted small">
                                <div><i class="bi bi-building"></i> {{ $showtime->cinema->name }}</div>
                                <div><i class="bi bi-clock"></i> {{ $showtime->show_time->format('d/m/Y H:i') }}</div>
                                <div><i class="bi bi-cash"></i> เริ่มต้น {{ number_format((float) $showtime->price) }} บาท
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="seat-row {{ $row['zone_start'] ? 'zone-start' : '' }}">
                                    ...
                                    @foreach ($row['seats'] as $seat)
                                        ...
                                        <button type="button"
                                            ...
                                            data-seat="{{ $seat }}" data-price="{{ $row['price'] }}"
                                            title="{{ $seat }} — {{ $row['zone'] }} {{ number_format($row['price']) }} บาท"
                                            style="--zone: {{ $row['color'] }}" @disabled($isBooked)>
                                            {{ $pos }}
                                        </button>
                                    @endforeach
                        ...
                        @foreach ($zones as $z)
                            <span>
                                <span class="legend-box"
                                    style="border:2px solid {{ $z['color'] }}; background:#fff;"></span>
